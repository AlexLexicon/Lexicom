...
public static class ServiceCollectionExtensions
{
     ...
     * 
     * for example: if you have some service 'MyService' that implements an 'IRequestHandler<MyRequest>'
     * and you send a new 'MyRequest', a new instance of 'MyService' will be created to handle that request.
     * 
     ...
     * then we un-register all handlers (IRequestHandler/INotificationHandler) that are implemented on view models
     ...
     * as soon as the view model would be garbage collected, it is without handing in this 'WeakViewModelRefrenceCollection' object.
     * 
     ...
     * in this case mediatR registers the 'INotificationHandler' as an 'INotificationHandler<TNotification>' instead of the actual concrete notificationt types
     * 
            ...
            if (viewModelRegistrationsForHandler.Any())
            {
                ...
                if (genericArguments.Any(ga => ga.Name is "TNotification"))
                {
                    ...
                    foreach (var viewModelRegistrationForHandler in viewModelRegistrationsForHandler)
                    {
                        ...
                        {
                            StaticSetupNotificationHandlersForViewModelsMethodInfo.MakeGenericMethod(viewModelRegistrationForHandler.ServiceType, handlerInterface).Invoke(null, new object[] { services });
                        }
                    }
                }
                else
                {
                    foreach (var viewModelRegistrationForHandler in viewModelRegistrationsForHandler)
                    {
                        ...
                        {
                            StaticSetupNotificationHandlersForViewModelsMethodInfo.MakeGenericMethod(viewModelRegistrationForHandler.ServiceType, handlerDescriptor.ServiceType).Invoke(null, new object[] { services });
                        }
                        else
                        {
                            StaticSetupRequestHandlersForViewModelsMethodInfo.MakeGenericMethod(viewModelRegistrationForHandler.ServiceType, handlerDescriptor.ServiceType).Invoke(null, new object[] { services });
                        }
                    }
                }
            }
    ...
    private static MethodInfo StaticSetupNotificationHandlersForViewModelsMethodInfo => _staticSetupNotificationHandlersForViewModelsMethodInfo ??= (typeof(ServiceCollectionExtensions).GetMethod(nameof(SetupNotificationHandlersForViewModels), BindingFlags.Static | BindingFlags.NonPublic) ?? throw new UnreachableException($"The method '{nameof(SetupNotificationHandlersForViewModels)}' was not found."));
    ...
    private static MethodInfo StaticSetupRequestHandlersForViewModelsMethodInfo => _staticSetupRequestHandlersForViewModelsMethodInfo ??= (typeof(ServiceCollectionExtensions).GetMethod(nameof(SetupRequestHandlersForViewModels), BindingFlags.Static | BindingFlags.NonPublic) ?? throw new UnreachableException($"The method '{nameof(SetupRequestHandlersForViewModels)}' was not found."));
